...
const REROLL_BUTTON_WIDTH: f32 = 64.0;
const REROLL_BUTTON_HEIGHT: f32 = 32.0;
static REROLL_BUTTON_SPRITE: Sprite = Sprite::new(16.0, 16.0, 64.0, 32.0);
static REROLL_BUTTON_DOWN_SPRITE: Sprite = Sprite::new(16.0, 48.0, 64.0, 32.0);
...
impl RerollButton {
    pub fn new() -> Self {
        RerollButton {
            button: Button::new(Rectangle {
                x: VIRTUAL_WIDTH / 2.0 - REROLL_BUTTON_WIDTH - 2.0,
                y: VIRTUAL_HEIGHT - DICE_Y_OFFSET + DICE_WIDTH_HEIGHT + 8.0,
                width: REROLL_BUTTON_WIDTH,
    ...
    pub fn draw(&self, d: &mut RaylibDrawHandle, texture: &Texture2D, font: &Font) {
        match self.down {
            true => REROLL_BUTTON_DOWN_SPRITE.draw(d, self.pos, texture),
            false => REROLL_BUTTON_SPRITE.draw(d, self.pos, texture)
        }
        
        d.draw_text_ex(font, "REROLL", self.pos + Vector2 { x: 2.0, y: 10.0 }, 9.0, 0.5, Color::WHITE);
    }
